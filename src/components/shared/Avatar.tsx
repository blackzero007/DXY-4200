...
export default function Avatar({
  ...
  return (
    <div className={`relative inline-block ${className}`}>
      ...
      <motion.div
        className={`${s.w} relative rounded-full flex items-center justify-center font-display text-white select-none`}
        style={{
          background: color,
        ...
        whileHover={!isCurrentTurn ? { scale: 1.1, y: -2 } : {}}
      ...
      {showCrown && isOwner && (
        ...
        >
          <Crown className={s.crown} style={{ filter: 'drop-shadow(0 1px 2px rgba(0,0,0,0.4))' }} />
        </motion.span>
      )}
      {!isWatcher && (
        <span
